...
const {RenciaDatabase} = require('../Functions/RenciaDatabase');
module.exports = {
  name: "isim",
  aliases: ["i", "name","k","e","kayıt"],
  ...
  function embed(msg) {
    ...
    let collector = await msg.createButtonCollector(filter, { time: 30000 })
      ...
      if(button.id === "MAN") {
      await RenciaDatabase.man(user, message.author)
      await RenciaDatabase.setusername(user, name_2, `<@&${settings.ManRole}>`)
      await button.think(true)
      await button.reply.edit(`${user} adlı kullanıcı başarıyla <@&${settings.ManRole}> rolüyle kayıt edildi`)
      }
      if(button.id === "WOMAN") {
      await RenciaDatabase.woman(user, message.author)
      await RenciaDatabase.setusername(user, name_2, `<@&${settings.WomanRole}>`)
      await button.think(true)
    ...
    await user.setNickname(name_2)
    RenciaDatabase.setusername(user, name_2, "İsim Değiştirme")
