...
let embed = new MessageEmbed().setColor("RANDOM").setAuthor(member.guild.name, member.guild.iconURL({dynamic: true})).setFooter("Adelicia ❤️ Rencia" + member.guild.name).setTimestamp()
if(security === "şüpheli") {
 ...
 channel.send(`
\`>\` Eternal'e Hoşgeldin Dostummm ${member}! Seninle birlikte ${member.guild.memberCount} kişiye ulaştık.
    
    \`>\` Hesabın \`${moment(member.user.createdAt).locale("tr").format("LLL")}\` tarihinde \`(${client.giris(member.user.createdAt)})\` açılmış.
    
      \`>\` Sunucu kurallarımız <#${settings.rulesChannel}> kanalında belirtilmiştir. Kurallara uymazsan cezasını Çekersin :D.
    
    \`>\` Tagımızı \`(${settings.Tag})\` alarak bizlere destek olabilirsin.
    
\`>\` Sol taraftaki \`<#976160138249515068>\` kanallarından birine girip \`isim-yaş\` belirterek kayıt olabilirsin.`)
};
});
...
client.login("OTU1MTYyNTIzNjA5NDg1MzY1.GgPChA.w7A_tBgW7xBeiap7HEQQTo27L1zRxpfwMbbCTI");
// ------------------[ END ]------------------ \\
